...
public class MergeSortAscending {
	public static double[] doMergeSortTwoSortedArrays(double[] array1, double[] array2) {
		int j = 0;
		int k = 0;
		int i = 0;
		...
		while (i < array1.length) {
			if (j >= array2.length || array1[i] <= array2[j]) { // choose the
																// smallest
																// value
				array[k++] = array1[i++];
			} else {
	...
	public static double[] doMergeSortOneNonSortedArray(double[] array) {
		if (array.length < 2) {
			return array;
		}
		...
		return doMergeSortTwoSortedArrays(doMergeSortOneNonSortedArray(array1), doMergeSortOneNonSortedArray(array2));
	}
}
